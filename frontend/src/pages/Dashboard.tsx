          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-sm">
            ...
            {user.telefoneContato && (
              <div>
                ...
                <p className="font-medium text-lg">{user.telefoneContato}</p>
              </div>
            )}
          </div>
        </CardContent>
            ...
            <CardContent>
              <div className="space-y-6">
                <div>
                  ...
                  <p className="text-gray-700 whitespace-pre-wrap">{user.descricao || 'Sem descrição.'}</p>
                </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <Card>
              ...
              <CardContent>
                ...
                <div className="space-y-2">
                  <h4 className="text-sm font-semibold text-gray-500 uppercase">Minhas Categorias</h4>
            ...
            <Card>
              ...
              <CardContent>
                ...
                <div className="space-y-2">
                  <h4 className="text-sm font-semibold text-gray-500 uppercase">Meus Produtos</h4>
                  {user.produtos && user.produtos.length > 0 ? (
                    <ul className="divide-y border rounded-lg">
                      {user.produtos.map(prod => (
                        <li key={prod.id} className="p-3 text-sm flex justify-between items-center">
                          <div>
                            <span className="font-medium">{prod.nome}</span>
                            {prod.marca && <span className="text-gray-400 ml-2">({prod.marca})</span>}
                          </div>
                          <span className="text-blue-600 font-semibold">R$ {prod.preco}</span>
                        </li>
                      ))}
                    </ul>
                  ) : (
                    <p className="text-sm text-gray-400 italic">Nenhum produto cadastrado.</p>
                  )}
                </div>
              </CardContent>
            </Card>
